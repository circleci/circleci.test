...
;; Portions Copyright (c) Circle Internet Services. All rights reserved

(ns
  ^{:author "Original authors: Stuart Sierra, with contributions and suggestions by
  Chas Emerick, Allen Rohner, and Stuart Halloway.
  Modified by CircleCI.",
...
"}
  ...
  (:require [clojure.template :as temp]
            [circleci.test.assert :refer (try-expr)]
...
(defmacro testing
  ...
  [string & body]
  `(binding [report/*testing-contexts* (conj report/*testing-contexts* ~string)]
     ~@body))
...
(defn- test*
  ...
  (when-let [t (:test (meta v))]
    (report/do-report {:type :begin-test-var, :var v})
...
(defn test-var
  ...
  (let [ns (-> v meta :ns)
    ...
    (when (:test (meta v))
      (binding [report/*testing-vars* (conj report/*testing-vars* v)]
        (let [start-time (System/nanoTime)]
          (try
            (each-fixture-fn (fn [] (test* v)))
            (catch Throwable e
              (report/do-report {:type :error,
                                 :message "Uncaught exception, not in assertion."
                                 :expected nil, :actual e}))
            (finally
              (let [stop-time (System/nanoTime)
                    elapsed (-> stop-time (- start-time) nanos->seconds)]
                (report/do-report {:type :end-test-var,
                                   :var v
                                   :elapsed elapsed})))))))))
...
(defn test-ns
  ...
  *report-counters*."
  [ns]
  (binding [report/*report-counters* (ref report/*initial-report-counters*)]
    (let [ns-obj (the-ns ns)]
      (report/do-report {:type :begin-test-ns, :ns ns-obj})
      ;; If the namespace has a test-ns-hook function, call that:
      (if-let [v (find-var (symbol (str (ns-name ns-obj)) "test-ns-hook"))]
        ((var-get v))
        ;; Otherwise, just test every var in the namespace.
        (test-all-vars ns-obj))
      (report/do-report {:type :end-test-ns, :ns ns-obj}))
...
(defn -main
  ...
  (let [nses (map read-string ns-strings)]
    (if-not (seq nses)
      ...
      (do
        (apply require :reload nses)
        (apply run-tests nses)))))
